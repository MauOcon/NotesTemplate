...
\section{Paquetes}
	...
	El paqute \tmplLatexPackage{babel} añade soporte multilenguaje. Adapta el documento a las convenciones de un lenguaje en específico.
	
	\begin{description}
		\item[Separación.] Carga la separación correcta entre palabras del lenguaje.
		\item[Traduce cadenas.] Cambia cadenas de texto como \textquote{Chapter}, \textquote{Table of Contents}, \textquote{References}, etc. al lenguaje objetivo.
		\item[Reglas tipográficas.] Aplica convenciones específicas del lenguaje para espaciado, puntuación y citas.
	...
	Permite especificar al usuario la codificación del archivo fuente
	
	\subsection{csquotes.}
	
	% Comillas básicas (se adaptan al idioma activo)
	\enquote{Texto entre comillas}
	% → "Texto entre comillas" (mexicano)
	% → "Texto entre comillas" (english)
	
	% Comillas anidadas (alterna estilo automáticamente)
	\enquote{Texto externo \enquote{texto interno} continúa}
	% → "Texto externo 'texto interno' continúa"
	
	% Comillas foráneas (fuerza estilo de otro idioma)
	\foreignquote{english}{This is quoted in English style}
	
	% Cita en bloque (para citas largas, usa indentación)
	\blockquote{Un párrafo largo que se formatea como cita en bloque.}
	
	% Comilla textual con cita bibliográfica
	\textquote[fuente]{Texto citado}
	
	% Cita con referencia (requiere biblatex)
	\textcquote{IEEEreferencias:Ref1}{Texto citado de la fuente}
	
	Según el autor, \enquote{la ciencia avanza \enquote{a pasos agigantados} cada día}.
	
	\foreignquote{english}{Knowledge is power.}
	...
	\blockquote{Este es un texto largo que se presenta como cita en bloque,	con indentación especial para distinguirlo del texto principal.}
...
\section{Definicioines.}

La definición se fibration \ref{def:fibration} se muestra a continuación
